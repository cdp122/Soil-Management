...
router.post('/registrarzona', validateToken, async (req, res) => {
    ...
    if (!req.body) { res.status(400).json({ error: "No se ha proporcionado informaci�n" }); return; }
...
router.post('/muestras', validateToken, async (req, res) => {
    ...
    if (!req.body.mat_org) { res.status(400).json({ error: "No se ha proporcionado el porcentaje de cantidad org�nica" }); return; }
    ...
    if (req.body.elems) {
        ...
        for (var elems in req.body.elems) {
            ...
            if (!elem.simb_elem) { res.status(400).json({ error: "No se ha proporcionado el s�mbolo del elemento en el registro " + (elems + 1) }); return; }
...
router.post('/variables', validateToken, async (req, res) => {
    ...
    if (!req.body.elems) { res.status(400).json({ error: "No se ha proporcionado informaci�n de las nuevas variables" }); return; }
...
router.put('/muestras', validateToken, async (req, res) => {
    ...
    //Si hay variables secundarias debe de comprobarse de la buena estructura del json
    if (req.body.elems) {
        console.log("RUTAS >> MUESTRAS > Se identificaron elementos en la actualizaci�n de muestras. Comprobando estructura de elementos...");
        var elem;
        for (var elems in req.body.elems) {
            elem = req.body.elems[elems];
            if (!elem.simb_elem) { res.status(400).json({ error: "No se ha proporcionado el s�mbolo del elemento en el registro " + (elems + 1) }); return; }
        ...
        console.log("RUTAS >> MUESTRAS > Estructura de elementos correcta. Iniciando con la actualizaci�n...");
    }
        ...
        await BDD.transaction(async (t) => {
            ...
            for (var elems in req.body.elems) {
                ...
                if (!elem.var_id) {
                    ...
                    else { throw new Error("No se encontr� la variable con id " + elem.var_id); }
                }
...
router.post('/register', async (req, res) => {
    if (!req.body) { res.status(400).json({ error: "No se ha proporcionado informaci�n" }); return; }
    if (!req.body["rol"] && typeof req.body["rol"] !== string) { res.status(400).json({ error: "No se ha proporcionado el rol o rol inv�lido" }); return; }
    if (!req.body["cedula"]) { res.status(400).json({ error: "No se ha proporcionado la c�dula" }); return; }
    ...
    if (!req.body["password"]) { res.status(400).json({ error: "No se ha proporcionado la contrase�a" }); return; }
    if (!req.body["telefono"]) { res.status(400).json({ error: "No se ha proporcionado el n�mero de tel�fono" }); return; }
...
router.post('/login', async (req, res) => {
    ...
    if (!req.body["cedula"]) { res.status(400).json({ error: "No se ha proporcionado la c�dula" }); return; }
    if (!req.body["password"]) { res.status(400).json({ error: "No se ha proporcionado la contrase�a" }); return; }
    console.log("RUTAS >> LOGIN > Iniciando sesi�n...");
    ...
    try {
        ...
        if (!usuario) {
            console.log("RUTAS >> LOGIN > No se encontr� el usuario");
        ...
        if (usuario.tipus_id != 1 && usuario.tipus_id != 2 && usuario.tipus_id != 3) {
            console.log("RUTAS >> LOGIN > Usuario de otro m�dulo detectado. Impidiendo inicio de sesi�n");
        ...
        if (match) {
            ...
            console.log("RUTAS >> LOGIN > Contrase�a incorrecta");
            res.status(401).json({ error: "Contrase�a incorrecta" });
        }
    } catch (error) {
        console.error("RUTAS >> LOGIN > Error al iniciar sesi�n:", error);
        res.status(500).json({ error: "Error al iniciar sesi�n", detalles: error.original?.detail || error.message});
    }
});
...
router.delete('/account', validateToken, async (req, res) => {
    if (!req.query.cedula) {
        res.status(400).json({ error: "No se proporcion� el id de la cuenta" });
        return;
    }
...
router.put('/password', validateToken, async (req, res) => {
    if (!req.body) { res.status(400).json({ error: "No se ha proporcionado informaci�n" }); return; }
    if (!req.body.cedula) { res.status(400).json({ error: "No se ha proporcionado la c�dula" }); return; }
    if (!req.body.password) { res.status(400).json({ error: "No se ha proporcionado la nueva contrase�a" }); return; }
    ...
    try {
        ...
        console.log("RUTAS >> CAMBIAR CONTRASE�A > Contrase�a cambiada correctamente");
    } catch (error) {
        console.error("RUTAS >> CAMBIAR CONTRASE�A > Error al cambiar la contrase�a:", error);
        res.status(500).json({ error: "Error al cambiar la contrase�a", detalles: error.original?.detail || error.message });
    }
});

//Recuperar la cuenta, primer paso. Encontrar la cuenta
router.get('/recover', async (req, res) => {
    if (!req.query.cedula) { res.status(400).json({ error: "No se ha proporcionado la c�dula" }); return; }
    if (!req.query.email) { res.status(400).json({ error: "No se ha proporcionado el correo" }); return; }
    if (!req.query.telefono) { res.status(400).json({ error: "No se ha proporcionado el n�mero de tel�fono" }); return; }
    ...
    try {
        ...
        if (!usuario) {
            res.status(404).json({ error: "Error: Datos incorrectos o inv�lidos" });
            return;
        }

        console.log("RUTAS >> RECUPERAR CUENTA [1] > Cuenta para recuperaci�n encontrada exitosamente");
        res.json({ valid: true });
    } catch (error) {
...
//Recuperar la cuenta, segundo paso. Cambiar la contrase�a
